...
close all;
%% setup
snow=300;     % number of snow flakes [0 .. 5000]
star=150;     % number of snow flakes [0 .. 5000]

%% Snow
%create some random 3D coordinates for the snow (amount as in setup above)
starX=(rand(star,1)*25-12.5);
starY=(rand(star,1)*25);
starZ=(rand(star,1)*27);
%Note:Some flakes will end up IN the tree but just can't be seen then
plot3(starX,starY,starZ,'yx')%plot coordinates as white snow flakes
hold on;
%% draw tree
